...
const STATE_STYLES = {
  ...
  queued: 'bg-gray-500/20 text-gray-600 dark:text-gray-300 border-gray-500/40',
}
...
const HunterMissionStatus = ({ missions = [] }) => (
  <div className="bg-gradient-to-br from-white/95 to-gray-50/90 dark:from-gray-800/80 dark:to-gray-900/60 backdrop-blur-lg border border-gray-200/80 dark:border-white/10 rounded-md shadow-xl p-4">
    <div className="flex items-center gap-2 mb-3">
      ...
      <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-200">Pivot Missions</h3>
      <InfoModalTrigger
      ...
      {missions.length > 0 && (
        <span className="ml-auto text-xs text-gray-500 dark:text-gray-400 font-mono">{missions.length}</span>
      )}
    </div>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                <span className="font-mono text-gray-900 dark:text-gray-100 truncate">{mission.missionId}</span>
                <span className={`text-xs px-2 py-0.5 rounded border ${STATE_STYLES[state] || STATE_STYLES.queued}`}>
                  {state}
                </span>
              </div>
              {mission.lastTask && (
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 truncate">{mission.lastTask}</p>
              )}
            </li>
          )
        })}

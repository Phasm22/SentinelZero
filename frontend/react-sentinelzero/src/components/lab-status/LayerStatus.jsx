      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
        ...
        <button
          ...
          className={`px-3 py-1 rounded-md text-sm font-medium transition-all duration-200 self-start ${
            filter === 'all'
              ? 'bg-gray-800/80 text-gray-100 border border-gray-600/50 shadow-lg'
          ...
          return (
            <button
              ...
              className={`group p-4 rounded-lg border backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl cursor-pointer ${
                ...
                isActive ? 'ring-1 ring-white/30 shadow-xl' : ''
              }`}
            >
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <div className="text-gray-100 font-semibold text-sm sm:text-base">{layer.name}</div>
                    <div className="card-meta">{layer.description}</div>
                  </div>
                </div>
                ...
                {[...Array(layer.data.total)].map((_, i) => (
                  <div
                    ...
                    className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${
                      i < layer.data.up
                        ? `${status === 'healthy' ? 'bg-green-400 shadow-sm shadow-green-400/50' :
                            status === 'warning' ? 'bg-yellow-400 shadow-sm shadow-yellow-400/50' :

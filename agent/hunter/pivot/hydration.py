...
def hydrate_seed(ip: str, scan_id: int | None) -> dict[str, Any]:
    ...
    empty: dict[str, Any] = {
        ...
        "rpc_audit": None, "rdp_recon": None, "source_scan_id": scan_id,
    }
    if not scan_id:
        return empty
    ...
    rdp_scripts = {
        v.get("id"): v.get("output", "")
        for v in host_vulns
        if v.get("id") in RDP_RECON_SCRIPT_IDS
    }
    rdp_recon = parse_rdp_scripts(rdp_scripts) if rdp_scripts else None

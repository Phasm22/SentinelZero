...
def hydrate_seed(ip: str, scan_id: int | None) -> dict[str, Any]:
    ...
    skip re-discovering ports/versions/http-recon it already has evidence for.
    ...
    """
    empty: dict[str, Any] = {"open_ports": [], "http_recon": None, "source_scan_id": scan_id}
    if not scan_id:
        return empty
    ...
    scripts = {
        ...
        if str(v.get("host") or "") == ip and v.get("id") in HTTP_RECON_SCRIPT_IDS
    }
    http_recon = parse_recon_scripts(scripts) if scripts else None

    return {"open_ports": open_ports, "http_recon": http_recon, "source_scan_id": scan_id}
